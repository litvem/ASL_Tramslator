...
class DataHandler:
    def __init__(self, db_file):
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS RETRAINING_DATA_7 (
                org_text TEXT,
                clean_text TEXT,
                start_time REAL,
                signer_id INTEGER,
    ...
    def insert_data(self, json_file):
        for row in json_file:
            ...
            self.cursor.execute('''INSERT INTO RETRAINING_DATA_7 (org_text, clean_text, start_time, signer_id, signer, start, end, file, label, height, fps, end_time, url, text, box, width) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 

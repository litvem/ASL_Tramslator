...
import pipes.Data_prepare_pipeline as DP


class VideoProcessorTests(unittest.TestCase):
    def setUp(self):
        # self.DATA_PATH = 'test2/MP_Data'
        # self.actions = np.array(['hello'])
        # self.sequence_length = 60
        # self.videos_folder = 'test2/videos'
        # self.DB_path = 'MS-ASL/data.db'
    ...
    def test_mediapipe_detection(self):
        ...
        image = cv2.cvtColor(cv2.imread('pipes/test2/0.png'), cv2.COLOR_BGR2RGB) # COLOR CONVERSION BGR 2 RGB
        ...
        _,results = DP.mediapipe_detection(cv2.imread("pipes/test2/0.png"), model)
    ...
    def test_save_frames(self):
        # Create test data
        file_name = 'pipes/test2/videos/0.mp4'
        sequence_length = 20
        path_to_np_video = 'pipes/test2/MP_data/hello'
    ...
    def test_save_videos(self):
        ...
        videos_address = 'pipes/test2/videos'
        np_address = os.path.join('pipes/test2/MP_data') 
        video_id = 1
        sequence_length = 20
